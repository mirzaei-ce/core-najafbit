...
###   This script attempts to download the signature file SHA256SUMS.asc from najafbit.org
...
WORKINGDIR="/tmp/najafbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://najafbit.org/bin/"
VERSIONPREFIX="najafbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'najafbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Najafbit Core binary release signing key installed?"
   fi
